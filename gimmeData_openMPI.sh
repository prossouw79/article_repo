...
then
	...
	mpirun -N 1 --machinefile  $machinefile sh ~/netSwitch/BCM_off.sh
	mpirun -N 1 --machinefile  $machinefile sh ~/netSwitch/CISCO_on.sh
	#mpirun -N 1 --machinefile  $machinefile sh ~/article_repo/helper_scripts/replace_hosts_cisco.sh
fi
...
then
	...
	mpirun -N 1 --machinefile $machinefile sh ~/netSwitch/CISCO_off.sh
	mpirun -N 1 --machinefile  $machinefile sh ~/netSwitch/BCM_on.sh
	#mpirun -N 1 --machinefile  $machinefile sh ~/article_repo/helper_scripts/replace_hosts_bcm.sh
fi

#kill any existing iperf3 servers
echo "Killing running iperf3 servers"
mpirun -N 1 --machinefile  $machinefile pkill iperf3
sleep 1

#start iperf3 servers in TCP mode
echo "Starting new TCP iperf3 servers"
mpirun -N 1 --machinefile  $machinefile $server_start &
sleep 1
		 ...
		 do
					...
					then
					  echo $COUNTER
						...
						mpirun -host "$wifi_ip_1" fping -c $test_time $wifi_ip_2 | tee -a $outfile

						echo "Running TCP BANDWIDTH TEST..."
						echo "#;"$COUNTER";TCP iperf3;"$wifi_ip_1";"$wifi_ip_2";"$test_time >> $outfile
						mpirun -host "$wifi_ip_1" iperf3 -c $wifi_ip_2 $iperf_tcp_args $test_time | tee -a $outfile

						echo "Running UDP BANDWIDTH TEST"
						echo "#;"$COUNTER";UDP iperf3;"$wifi_ip_1";"$wifi_ip_2";"$test_time >> $outfile
						mpirun -host "$wifi_ip_1" iperf3 -c $wifi_ip_2 $iperf_udp_args | tee -a $outfile
